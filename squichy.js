...
try {
  ...
  const pushname = m.pushName || "Devilcrash Beta1";
  ...
  const time = moment.tz("Zimbabwe/Harare").format("HH:mm:ss");
...
if (getSetting(m.sender, "autobio", true) && (Date.now() - global._lastAutobioUpdate > AUTOBIO_COOLDOWN_MS)) {
    ...
    prim.updateProfileStatus(`Devilcrush Bug Connected ☑️`).catch(_ => _)
}
...
            url: "https://t.me/sourcecodemarket666",
            contextInfo: {
...
> ╚════── • ──═══╝
`
        const imageUrl = "https://i.ibb.co/kVjS268F/x.jpg";
...
case 'frz-wa': {
    ...
    if (args.length !== 1) {
        return reply(
            `*❌ 𝙸𝙽𝚅𝙰𝙻𝙸𝙳 𝚄𝚂𝙰𝙶𝙴.*\n\n` +
            `*— 𝚄𝚂𝙰𝙶𝙴: ${prefix+command} 263xxx*\n` +
            `*𝙴𝚇𝙰𝙼𝙿𝙻𝙴: ${prefix+command} 263714373955*\n\n` +
